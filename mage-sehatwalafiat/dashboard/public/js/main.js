...
if (!deviceId) {
  ...
  console.error('KRITIS: activeUserId tidak ditemukan di body!');
...
// New Update
socket.on('update-status', (data) => {
  if (data.deviceId.toString() !== deviceId) return; // Hanya untuk perangkat ini
  ...
  if (elDeviceStatusBadge) {
    elDeviceStatusBadge.className = `badge fs-6 ${statusClass}`;
  }
  if (elDeviceStatusIcon) {
    elDeviceStatusIcon.className = `fa-solid ${iconClass} me-2`;
  }
});
...
const elRealtimeStandby = document.getElementById('realtime-standby');
const elEwsStandby = document.getElementById('ews-standby');
const elProfileStandby = document.getElementById('profile-standby');

// Referensi Sesi Aktif (Ini semua ada di dalam HTML, tapi ter-d-none)
const elRealtimeContent = document.getElementById('realtime-content-wrapper');
const elEwsContent = document.getElementById('ews-content-wrapper');
const elProfileContent = document.getElementById('profile-content-wrapper');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  console.log("Halaman dashboard dimuat dengan sesi aktif.");
  ...
  if (typeof currentUser !== 'undefined' && currentUser) {
    ...
    // (Panggil fungsi untuk mengisi form profil)
    // loadProfileData(currentUser, userConditions);
  }
...
function updateStatisticsUI(stats, logs) {
  if (!elStatAvgHr) return; // Pengaman
  ...
  elStopSessionBtn.addEventListener('click', async () => {
    if (confirm('Apakah Anda yakin ingin menghentikan sesi pasien ini?')) {
      ...
      try {
        ...
        window.location.href = `/link-device/${deviceId}`;
...
function loadProfileData(sessionData) {
  if (!elProfileForm) return;
  ...
  // (Anda perlu menambahkan logika untuk mengisi checkbox kondisi medis)
}

function showModalStatus(message, isError = false) {
  if (!elModalGlobalStatus) return;
  elModalGlobalStatus.textContent = message;
  elModalGlobalStatus.className = isError ? 'me-auto text-danger' : 'me-auto text-success';
}
